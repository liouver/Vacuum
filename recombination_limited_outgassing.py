...
def time_dependent_outgass(D, time, C0, K, S):
...
def main():
    ...
    for i in range(np.size(T)):
        q, u_data = time_dependent_outgass(D[i], time, C0, K[i], S[i])
        q_data[:, i] = np.array(q) * R * T[i] / (133.322 * 6.022 * 10**20)
    q, u_data = time_dependent_outgass(D[5], time, C0, K[5], S[5])
    ...
    plt.xlim([0, 0.3])
    plt.xlabel('depth (cm)')
    plt.ylabel('Hydrogen concentration ($torr \cdot L \cdot cm^{-3}$)')
